...
Voor deze opdracht werd gekeken naar de data van een collega-laborant. Deze analyse werd reproduceerbaar uitgewerkt in een RMarkdown bestand. De data is afkomstig van het HU lectoraat Innovative Testing in Life Sciences & Chemistry.
...
```
\
De data werd goed ingelezen en was al in tidy format gezet, er hoefde dus niks aangepast te worden aan het format van de data.

## Data types voor RawData, compName en compConcentration
Voor deze ananlyse werd gebruik gemaakt van de variabelen RawData, compName en compConcentration. Eerst werd gecontroleerd of het data type voor deze variabelen klopt zodat er een scatterplot kon worden gemaakt. RawData en comConcentration moesten het data type numeric zijn en compName moest character zijn.
...
De gemiddelde waarde voor de negatieve controle (controlNegative) moest gelijk worden gesteld aan de waarde 1. De andere meetwaarden (experiment, controlPositive en controlVehicleA) werden uitgedrukt als een fractie daarvan. 
...
Om de IC50 concentratie van de verschillende chemicaliën te bepalen, mowr er een dose-response analyse met een log-logistisch model worden uitgevoerd. In Rstudio is hiervoor het drc package beschikbaar. Om deze analyse uit te kunnen voeren, moeten de volgende stappen worden uitgevoerd: \
